defmodule Doble9Engine.UI.Player do
  ...
  def render %{axis: :x, player: %{dominoes: dominoes}, window: %{width: width}} = player do
    render merge player, %{label_at: :left, dy: 0, dx: width(:y), left: div(width - (width(:y) * length(dominoes)), 2)}
  end

  def render %{axis: :y, player: %{dominoes: dominoes}, window: %{height: height}} = player do
    render merge player, %{label_at: :top, dx: 0, dy: height(:x), top: top = div(height - (height(:x) * length(dominoes)), 2)}
  end
